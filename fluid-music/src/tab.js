...
const parseScore = function(object, rhythm, noteLibrary, startTime, vPattern, vLibrary, tracksObject, trackKey) {
  ...
  if (Array.isArray(object)) {
    ...
    const results = [];
    for (let o of object) {
      let result = parseScore(o, rhythm, noteLibrary, startTime + duration, vPattern, vLibrary, tracksObject, trackKey);
      results.push(result);
    ...
    for (let [key, val] of Object.entries(object)) {
      ...
      let result = parseScore(val, rhythm, noteLibrary, startTime, vPattern, vLibrary, tracksObject, key);
...
/**
 * These keys are reserved, and cannot be used for patterns.
 */
const reservedKeys = {
  r: null,
  v: null,
