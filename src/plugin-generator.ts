 ...
 * @param config.className Optionally override the report's pluginName
 * @param config.internal Set this to `true` for any plugins that are bundled
 * with the main `fluid-music` library. By default, the plugin modules generated
 * by `generatePluginModule` import their fluid-music dependencies with from the
 * `fluid-music` module. The plugins that are bundled with fluid-music, should
 * import their deps from local typescript modules.
 */
export function generatePluginModule(report : any, config : { className?: string, extraMethods?: string, internal?: boolean } = {}) {
  ...
  const internalImports = `import { PluginType, FluidPlugin } from '../FluidPlugin';
import { PluginAutomation } from '../techniques';`
  const externalImports = `import { PluginType, FluidPlugin, techniques } from 'fluid-music';
const { PluginAutomation } = techniques;`

  let output = `${config.internal ? internalImports : externalImports}
